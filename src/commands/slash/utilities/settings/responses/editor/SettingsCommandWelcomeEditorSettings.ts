...
export default class SettingsCommandWelcomeEditorSettings {
  ...
  public static async response(client: NoirClient, interaction: ModalMessageModalSubmitInteraction, id: string, type: 'guild_join' | 'guild_left' | 'direct_join'): Promise<void> {
    ...
    if (colorInput) {
      messageData.embed.rawColor = client.utils.formatValue(colorInput)
      messageData.embed.color = client.utils.formatColor(client.utils.formatValue(colorInput))
    }

    if (descriptionInput) {
      messageData.embed.description = client.utils.formatValue(descriptionInput)
    }

    if (imageInput) {
      messageData.embed.rawImage = client.utils.formatValue(imageInput)
      messageData.embed.image = client.utils.formatImage(interaction, client.utils.formatValue(imageInput))
    }

    if (thumbnailInput) {
      messageData.embed.rawThumbnail = client.utils.formatValue(thumbnailInput)
      messageData.embed.thumbnail = client.utils.formatImage(interaction, client.utils.formatValue(thumbnailInput))
    }
